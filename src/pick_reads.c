...
int pickreads(ref_info *ref_info, options_rf *opt, sam **sds) {
	...
	size_t **rchar = NULL;
	rchar = malloc(N_FILES * sizeof(*rchar));
	unsigned int i, j, m;
...
// extract the region agter using samtools, do samtools faidx reference.fasta first, then samtools faidx ref.fasta name:1-100
void extract_ref(const char *samtools_command, char *region, const char *ref_file, const char *ext_rf) {
	
	unsigned int cmd_len = strlen(samtools_command) + strlen(ref_file) + strlen(" faidx ") + strlen(region) + strlen(" -o ") + strlen(ext_rf);
