...
call_ampliclust <- function(ampliclust_command = NULL, fastq_file = NULL) {
  ...
  res <- .Call("r_ampliclust_init", ampliclust_command, fastq_file)
  names(res) <- c("reads", "dim")
  reads <- matrix(res$reads, ncol = res$dim[2], byrow = TRUE)
  return(reads)
}
...
# haps <- call_ampliclust(ampliclust_command, fastq_file)
# ampliclust_command = "../../../../../Downloads/amplici/run_ampliclust"
