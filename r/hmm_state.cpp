...
List aux_noN_S2(IntegerVector sum_site, IntegerVector hap_site, List opt) {
  int n_row;
  double sum;
  ...
  if (num == 1) {
    ...
    if (hap_site[0] == -1) {
      ...
      if(sum_site[0]/sum >= 0.45) {
        ...
        List out = aux_noN_S2(sum_site_cleaned, hap_site_cleaned, opt);
        haplotype[0] = out["temp"];
        n_row = out["n_row"];
      }
    } else {
      List out = aux_noN_S3(sum_site, hap_site, opt);
      haplotype[0] = out["temp"];
      n_row = out["n_row"];
    }
  }
